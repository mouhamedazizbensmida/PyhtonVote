...
st.title("Automatic Concurrent POST Request Sender")
...
last_resp_placeholder = st.empty()

# Display initial metrics
status_placeholder.text(f"Status: {st.session_state.status}")
success_placeholder.metric("✅ Successful Requests", st.session_state.success_count)
failure_placeholder.metric("❌ Consecutive Failures", st.session_state.failure_count)

# Show last 10 responses
st.subheader("Last 10 Responses")
for resp in st.session_state.last_responses[-10:]:
    try:
...
def send_request(url, json_body):
    try:
        resp = requests.post(url, json=json_body, timeout=20000)
...
if st.session_state.running:
    batch_size = 100  # send 100 requests concurrently
    st.session_state.last_responses = []

    with ThreadPoolExecutor(max_workers=batch_size) as executor:
        futures = [executor.submit(send_request, api_url, body) for _ in range(batch_size)]

        for future in as_completed(futures):
            status, resp_text = future.result()
            st.session_state.last_responses.append(resp_text)
            ...
            status_placeholder.text(f"Status: {st.session_state.status}")
            try:
                last_resp_placeholder.json(json.loads(resp_text))
            except:
                last_resp_placeholder.text(resp_text)
    ...
    # Optional: rerun to start next batch automatically if still running
    if st.session_state.running:
