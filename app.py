...
default_body = {
    ...
    "recaptcha": "your-long-recaptcha-token"
}

# --- Streamlit UI ---
st.title("Parallel POST Request Sender")
...
# Number of requests
N = st.number_input("Number of Requests (N)", min_value=1, value=50, step=1)
MAX_WORKERS = st.number_input("Max Parallel Workers", min_value=1, value=10, step=1)
...
def run_requests():
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            if not st.session_state.running:  # Stop if user pressed stop
                break
...
st.metric("❌ Failure", st.session_state.failure_count)
